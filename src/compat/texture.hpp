...
#define BOE_COMPAT_TEXTURE_HPP
	...
	namespace sf {
		...
		private:
			unsigned int width_;
			unsigned int height_;
			std::string filename_; // Store filename as texture ID for JavaScript
		...
		public:
			...
			bool loadFromFile(const std::string& filename) {
				filename_ = filename;
				#ifdef __EMSCRIPTEN__
				// Pre-load image - dimensions will be stored when image loads
				EM_ASM_({
					...
					if (!Module.textureCache[filename]) {
						try {
							...
							img.onload = function() {
								...
								Module.textureDimensions[filename] = dims;
								console.log('Texture loaded: ' + filename + ' ' + img.naturalWidth + 'x' + img.naturalHeight);
							};
							img.src = url;
				...
				// Use stub dimensions - actual used in draw code
				width_ = 100;
				height_ = 100;
				#endif
				return true;
			...
			ImageSize getSize() const {
				#ifdef __EMSCRIPTEN__
				// Query actual dimensions from JavaScript (two separate calls to avoid BigInt)
				unsigned int w = EM_ASM_INT({
					var filename = UTF8ToString($0);
				...
				unsigned int h = EM_ASM_INT({
					var filename = UTF8ToString($0);
					if (Module.textureDimensions && Module.textureDimensions[filename]) {
						return Module.textureDimensions[filename].height;
					}
					return $1;
				}, filename_.c_str(), height_);
