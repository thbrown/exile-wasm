...
static void loadImageToRenderTexture(sf::RenderTexture& tex, std::string imgName) {
	...
	// Use hardcoded dimensions for WASM since images load asynchronously
	// and texture dimensions aren't available immediately
	...
	auto it = sizes.find(imgName);
	int w = (it != sizes.end()) ? it->second.first : 300;
